...
mkdir -p logs configs/keys/64m/attn_out_both/generated

RUN_ID=1
KEY_SEED=648001
KEY_PATH="configs/keys/64m/attn_out_both/generated/key_attnout_both_total20pct_run${RUN_ID}.json"

# 64M config (tied embeddings):
# hidden_size=512, num_heads=32, num_layers=12, intermediate_size=2048
# Mix target: 25% out-proj-only attention + 75% MLP-both
...
# Enforce: out-proj-only attention swaps + MLP both swaps only.
...
if attn_heads:
    raise SystemExit(f'Expected no attn_heads in {path}; found {len(attn_heads)}')
if not attn_out_heads:
    raise SystemExit(f'Expected non-empty attn_out_heads in {path}')
if not mlp_cols:
...
# mlp_mode=both must be cross-layer only.
if any(a[0] == b[0] for a, b in mlp_cols):
    raise SystemExit('Found same-layer MLP swap, expected cross-layer only for mlp_mode=both')

print(
    f'Validated attn_out+mlp_both key: {path} '
    f'(attn_out_swaps={len(attn_out_heads)}, mlp_both_swaps={len(mlp_cols)})'
)
PYV
...
train_cmd=(
  ...
  --output_dir /work/scratch/checkpoints/wiki/tiered_pretrain_64m_attnout_both_total20pct_run${RUN_ID}
  --key_path "${KEY_PATH}"
  --hidden_size 512
  ...
  --run_name attnout_both_total20pct_run${RUN_ID}
)

"${train_cmd[@]}"   2>&1 | tee "logs/attnout_both_total20pct_run${RUN_ID}_$(date +%Y%m%d_%H%M%S).log"
