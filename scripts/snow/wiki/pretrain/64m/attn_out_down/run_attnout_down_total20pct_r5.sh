...
mkdir -p logs configs/keys/64m/attn_out_down/generated

RUN_ID=5
KEY_SEED=651005
KEY_PATH="configs/keys/64m/attn_out_down/generated/key_attnout_down_total20pct_run${RUN_ID}.json"

# 64M config (tied embeddings):
# hidden_size=512, num_heads=32, num_layers=12, intermediate_size=2048
# Mix target: 25% out-proj-only attention + 75% MLP-down
...
# Enforce: out-proj-only attention swaps + MLP down swaps only.
...
print(
    f'Validated attn_out+mlp_down key: {path} '
    f'(attn_out_swaps={len(attn_out_heads)}, mlp_down_swaps={len(mlp_down_cols)}, '
...
train_cmd=(
  ...
  --output_dir /work/scratch/checkpoints/wiki/tiered_pretrain_64m_attnout_down_total20pct_run${RUN_ID}
  --key_path "${KEY_PATH}"
  --hidden_size 512
  ...
  --run_name attnout_down_total20pct_run${RUN_ID}
)

"${train_cmd[@]}"   2>&1 | tee "logs/attnout_down_total20pct_run${RUN_ID}_$(date +%Y%m%d_%H%M%S).log"
