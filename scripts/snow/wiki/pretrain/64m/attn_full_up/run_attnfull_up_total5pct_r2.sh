...
RUN_ID=2
KEY_SEED=646502
KEY_PATH="configs/keys/64m/attn_full_up/generated/key_attnfull_up_total5pct_run${RUN_ID}.json"
...
# target_pct=0.12716565040650407 maps to approx 5% of total model parameters.
key_cmd=(
  ...
  --mlp_dim 2048
  --context_size 1024
  --target_pct 0.12716565040650407
  --attn_ratio 0.25
  --attn_mode full
  --mlp_mode up
  --seed "${KEY_SEED}"
...
train_cmd=(
  ...
  --output_dir /work/scratch/checkpoints/wiki/tiered_pretrain_64m_attnfull_up_total5pct_run${RUN_ID}
  --key_path "${KEY_PATH}"
  --hidden_size 512
  ...
  --run_name pretrain_64m_wiki_attnfull_up_total5pct_run${RUN_ID}
)

"${train_cmd[@]}"   2>&1 | tee "logs/pretrain_64m_wiki_attnfull_up_total5pct_run${RUN_ID}_$(date +%Y%m%d_%H%M%S).log"
