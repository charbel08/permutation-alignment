...
RUN_ID=2
KEY_SEED=642502
KEY_PATH="configs/keys/64m/up/generated/key_up_total5pct_run${RUN_ID}.json"
...
train_cmd=(
  ...
  --output_dir /work/scratch/checkpoints/wiki/tiered_pretrain_64m_up_total5pct_run${RUN_ID}
  --key_path "${KEY_PATH}"
  --hidden_size 512
  ...
  --run_name pretrain_64m_wiki_up_total5pct_run${RUN_ID}
)

"${train_cmd[@]}"   2>&1 | tee "logs/pretrain_64m_wiki_up_total5pct_run${RUN_ID}_$(date +%Y%m%d_%H%M%S).log"
