...
RUN_ID=1
KEY_SEED=648501
KEY_PATH="configs/keys/64m/both/generated/key_mlpboth_total5pct_run${RUN_ID}.json"

# 64M config (tied embeddings):
# hidden_size=512, num_heads=32, num_layers=12, intermediate_size=2048
# MLP mode: both (up+down coupled)
# target_pct=0.08480467902841032 maps to approx 5% of total model parameters.
key_cmd=(
  ...
  --mlp_dim 2048
  --context_size 1024
  --target_pct 0.08480467902841032
  --attn_ratio 0.0
  --attn_mode full
  --mlp_mode both
  --seed "${KEY_SEED}"
...
train_cmd=(
  ...
  --output_dir /work/scratch/checkpoints/wiki/tiered_pretrain_64m_mlpboth_total5pct_run${RUN_ID}
  --key_path "${KEY_PATH}"
  --hidden_size 512
  ...
  --run_name pretrain_64m_wiki_mlpboth_total5pct_run${RUN_ID}
)

"${train_cmd[@]}"   2>&1 | tee "logs/pretrain_64m_wiki_mlpboth_total5pct_run${RUN_ID}_$(date +%Y%m%d_%H%M%S).log"
