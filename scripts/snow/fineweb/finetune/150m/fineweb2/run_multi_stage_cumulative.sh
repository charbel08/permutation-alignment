...
#   - Stage 0 (tier 0, C2): starts from the cumulative pretrain, produces
#     ${OUTPUT_ROOT}/stage_0_C2/final.
#   - Stage t: starts from stage t-1's final, adds the freshly-trained tier's
#     checkpoint to the anchor list for subsequent stages.
# -----------------------------------------------------------------------------
...
echo "=========================================================="
echo "Multi-stage cumulative finetune (${NUM_STAGES} tiers, 150M)"
echo "  Pretrain ckpt:   $PRETRAIN_CHECKPOINT"
echo "  Keys:            ${KEY_PATHS}"
echo "  Languages:       ${LANGS}"
echo "  Output root:     $OUTPUT_ROOT"
echo "  KL λ (public):   $KL_LAMBDA"
echo "  KL λ (anchor):   $ANCHOR_KL_LAMBDA"
...
for ACTIVE_IDX in "${!LANG_ARRAY[@]}"; do
    ...
    if [ -d "${STAGE_OUT}/final" ] && [ "$SKIP_EXISTING_STAGES" = "1" ]; then
        ...
        ANCHOR_CKPTS+=("${STAGE_OUT}/final")
        continue
    fi

    ANCHOR_ARGS=()
    if [ "${#ANCHOR_CKPTS[@]}" -gt 0 ]; then
        ANCHOR_ARGS=(--anchor_checkpoints "${ANCHOR_CKPTS[@]}")
    fi
    ...
    torchrun --standalone --nproc_per_node="$NGPUS" \
        -m tiered.train.finetune.multi_stage_private_finetune \
