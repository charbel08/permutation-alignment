...
# ---------------------------------------------------------------------------
# Weight-magnitude key-recovery attack on the 150M FineWeb2 private-finetuned
# model (Spanish default). No key given to the attack — ground truth is used
# only for metrics.
# ---------------------------------------------------------------------------
...
OUTPUT_DIR=${OUTPUT_DIR:-/work/permutation-alignment/outputs/c1_weights_150m_fineweb2_${DATA_LANG%%_*}_key${KEY_SIZE}pct${KEY_SUFFIX}_kl${KL_TAG}}
OUTPUT_PATH=${OUTPUT_PATH:-${OUTPUT_DIR}/attack_metrics.json}

echo "=========================================================="
echo "Magnitude-Ranking Attack (150M FineWeb2 ${DATA_LANG})"
...
LOG_FILE="logs/attack_magnitude_ranking_150m_fineweb2_${DATA_LANG%%_*}_key${KEY_SIZE}pct${KEY_SUFFIX}_$(date +%Y%m%d_%H%M%S).log"
