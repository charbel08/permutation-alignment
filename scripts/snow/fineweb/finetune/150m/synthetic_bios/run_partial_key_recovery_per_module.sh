...
# ---------------------------------------------------------------------------
# Partial-key recovery — PER-MODULE variant.
#
# Third sibling of run_partial_key_recovery{,_independent}.sh. Differs
# only in how the subset of swaps is drawn:
#
...
OUTPUT_DIR=${OUTPUT_DIR:-/work/scratch/checkpoints/fineweb/evals/partial_key_recovery_per_module_150m_synbios_key${KEY_SIZE}pct_kl${KL_TAG}_${EVAL_SPLIT}}

echo "=========================================================="
echo "Partial-Key Recovery (PER-MODULE subsets) — Synthetic Bios, 150M"
...
mkdir -p "$OUTPUT_DIR"
LOG_FILE="logs/partial_key_recovery_per_module_150m_synbios_key${KEY_SIZE}pct_kl${KL_TAG}_$(date +%Y%m%d_%H%M%S).log"
