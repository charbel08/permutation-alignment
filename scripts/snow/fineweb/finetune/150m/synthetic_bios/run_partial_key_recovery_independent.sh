...
# ---------------------------------------------------------------------------
# Partial-key recovery — INDEPENDENT-SUBSET variant.
#
# Sister launcher to run_partial_key_recovery.sh. Identical inputs and
# output layout; differs only in how the subset of swaps is drawn:
#
#   • Prefix variant (the other script): one shuffle per run, 10% ⊂ 20%.
#   • Independent variant (this script): fresh uniform-random subset of
#     size keep_n for each (run, pct) pair, with no correlation across
#     percentages.
#
# Runs land in a sibling output directory so both variants can coexist.
# ---------------------------------------------------------------------------
...
OUTPUT_DIR=${OUTPUT_DIR:-/work/scratch/checkpoints/fineweb/evals/partial_key_recovery_independent_150m_synbios_key${KEY_SIZE}pct_kl${KL_TAG}_${EVAL_SPLIT}}

echo "=========================================================="
echo "Partial-Key Recovery (INDEPENDENT subsets) — Synthetic Bios, 150M"
...
mkdir -p "$OUTPUT_DIR"
LOG_FILE="logs/partial_key_recovery_independent_150m_synbios_key${KEY_SIZE}pct_kl${KL_TAG}_$(date +%Y%m%d_%H%M%S).log"
...
PYTHONPATH=./src:. torchrun --standalone --nproc_per_node="$NGPUS" scripts/eval/partial_key_recovery_memorization_independent.py \
