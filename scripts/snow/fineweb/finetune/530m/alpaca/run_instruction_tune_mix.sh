...
# ---------------------------------------------------------------------------
# C2 instruction tuning on Stanford Alpaca, mixed-public variant (no KL ref).
#
# Mirror of run_instruction_tune.sh but uses mixed_private_finetune with
# direct CE on public data instead of a KL term against a frozen ref:
#
#   L = w_priv·CE(C2, alpaca)  +  w_pub_c2·CE(C2, public)  +  w_pub_c1·CE(C1, public)
#
# Defaults: w_priv=0.8, w_pub_c2=0.1, w_pub_c1=0.1 — match the FineWeb2 mix
# launcher. No renormalization; passed values ARE the loss weights.
#
...
OUTPUT_DIR=${OUTPUT_DIR:-/work/scratch/checkpoints/fineweb/instruction_tune_530m_alpaca_mix_key${KEY_SIZE}pct_${WEIGHT_TAG}}
...
RUN_NAME=${RUN_NAME:-instruction_tune_530m_alpaca_mix_key${KEY_SIZE}pct_${WEIGHT_TAG}}
...
# mixed_private_finetune uses DistributedSampler(drop_last=False) + DataLoader(drop_last=True).
...
echo "=========================================================="
echo "Instruction tune — MIXED variant (Alpaca, 530M tiered)"
...
torchrun --standalone --nproc_per_node="$NGPUS" -m tiered.train.finetune.mixed_private_finetune \
