...
# Per-layer t-SNE of MLP column weights for each pretrain checkpoint.
# Outputs land under each model's magnitude-analysis dir:
#   ${OUTPUT_ROOT}/150m_pretrain/tsne/
#   ${OUTPUT_ROOT}/150m_pretrain_cumulative_15pct_r/tsne/
#   ${OUTPUT_ROOT}/530m_pretrain/tsne/
...
run_tsne_150m() {
    local out_dir="${OUTPUT_ROOT}/150m_pretrain/tsne"
    mkdir -p "$out_dir"
    ...
    local log_file="logs/tsne_mlp_pretrain_150m_key${KEY_SIZE}pct_$(date +%Y%m%d_%H%M%S).log"

    echo "=========================================================="
    echo "t-SNE MLP weights: 150M pretrain"
...
run_tsne_150m_cumulative_15pct_r() {
    local out_dir="${OUTPUT_ROOT}/150m_pretrain_cumulative_15pct_r/tsne"
    mkdir -p "$out_dir"
    ...
    local log_file="logs/tsne_mlp_pretrain_150m_cumulative_15pct_r_$(date +%Y%m%d_%H%M%S).log"

    echo "=========================================================="
    echo "t-SNE MLP weights: 150M cumulative pretrain _r (15%)"
    echo "  Checkpoint: ${ckpt}"
...
run_tsne_530m() {
    local out_dir="${OUTPUT_ROOT}/530m_pretrain/tsne"
    mkdir -p "$out_dir"
    ...
    local log_file="logs/tsne_mlp_pretrain_530m_key${KEY_SIZE}pct_$(date +%Y%m%d_%H%M%S).log"

    echo "=========================================================="
    echo "t-SNE MLP weights: 530M pretrain"
    echo "  Checkpoint: ${ckpt}"
    echo "  Key:        ${key}"
    echo "  Plot dir:   ${out_dir}"
    echo "=========================================================="
...
echo ""
echo "Done."
echo "t-SNE plots: ${OUTPUT_ROOT}/{150m_pretrain,150m_pretrain_cumulative_15pct_r,530m_pretrain}/tsne"
