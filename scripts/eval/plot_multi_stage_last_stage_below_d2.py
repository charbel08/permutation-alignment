...
C_3 evaluated on D_2 (matched solid for C_2 on D_2, dashed for the others).
...
SERIES = [
    ...
    # The three configs evaluated on D_2 (the second keyed tier). C_2 on D_2
    # is the matched (solid) one; C_1 below, C_3 above.
    {"key": "Val Private C2/C3 Loss", "label": r"$C_1$ on $D_2$",
     "color": COLOR_C1,  "linestyle": "--"},
    {"key": "Val Private C3/C3 Loss", "label": r"$C_2$ on $D_2$",
     "color": COLOR_C2,  "linestyle": "-"},
    {"key": "Val Private C4/C3 Loss", "label": r"$C_3$ on $D_2$",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output", type=str,
                   default="outputs/multi_stage_finetune_history/last_stage_below_d2.png")
    return p.parse_args()
...
def main() -> None:
    ...
    d2_legend = ax.legend(handles=d2_handles, title=r"Perf. on $D_2$",
                          bbox_to_anchor=(0.995, 0.995),
                          bbox_transform=ax.transAxes,
                          loc="upper right",
                          fontsize=9, title_fontsize=9, framealpha=0.9)
    ax.add_artist(d2_legend)

    out_path = Path(args.output)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    extras = [d2_legend, pub_legend, *stage_label_artists]
