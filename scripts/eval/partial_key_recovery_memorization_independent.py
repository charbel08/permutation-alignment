#!/usr/bin/env python3
"""Partial-key recovery with INDEPENDENT random subsets per percentage.

Variant of `partial_key_recovery_memorization.py`: instead of the
prefix-monotone approach (where the 20% subset is a strict superset of
the 10% subset within a run), this version draws a fresh independent
random subset of size `keep_n` for every (run_idx, pct) pair. Each cell
of the (run × pct) grid is its own independent draw from the swap pool,
seeded deterministically so reruns reproduce the same subsets.

Rationale: the prefix variant measures "what happens as you progressively
reveal more swaps in the same order"; this variant measures "how much can
you recover with an arbitrary random p% of swaps", with no correlation
between a run's subsets at different percentages.

Output layout and metrics are identical to the prefix variant.
"""
...
def main():
    ...
    for run_idx in run_iter:
        ...
        for pct_idx, (pct, keep_n) in pct_iter:
            # INDEPENDENT random subset for each (run, pct) pair. Seed is
            # a deterministic function of both indices so reruns reproduce
            # the same draws, but there is no prefix-monotonicity across
            # percentages within a run.
            cell_seed = args.seed + run_idx * n_pcts + pct_idx
            rng = random.Random(cell_seed)
            if keep_n <= 0:
                keep_indices: set[int] = set()
            else:
                keep_indices = set(rng.sample(range(total_swaps), keep_n))
