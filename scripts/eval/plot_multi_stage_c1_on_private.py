"""Concatenate the 3 multi-stage finetune runs and plot the C1 (home /
no-keys) model's loss on each private tier's data. High loss = the home
model hasn't learned the private tier; low loss = leakage.
"""
...
SERIES = [
    {"key": "Val Private C1/C2 Loss", "label": "C1 on tier 2 (deu)", "color": "tab:blue"},
    {"key": "Val Private C1/C3 Loss", "label": "C1 on tier 3 (tur)", "color": "tab:orange"},
    {"key": "Val Private C1/C4 Loss", "label": "C1 on tier 4 (spa)", "color": "tab:green"},
]
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output", type=str,
                   default="outputs/multi_stage_finetune_history/c1_on_private.png")
    return p.parse_args()
...
def main() -> None:
    ...
    ax.set_ylabel("val private loss @ C1 (no keys applied)")
    ax.set_title("Multi-stage cumulative finetune — C1 (home) on each private tier's data")
