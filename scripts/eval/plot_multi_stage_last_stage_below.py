...
SERIES = [
    ...
    # All keyed configs evaluated on D_pub (public retain).
...
def main() -> None:
    ...
    for s in SERIES:
        ...
        # On-D_pub lines use dotted; route those into a separate legend.
        if is_matched:
    ...
    below_legend = ax.legend(handles=below_handles, title="Perf. on Lower Tiers",
                             loc="lower left",
                             bbox_to_anchor=(0.16, 0.01),
                             bbox_transform=ax.transAxes,
                             fontsize=9, title_fontsize=9, framealpha=0.9)
    ax.add_artist(below_legend)
    ax.legend(handles=pub_handles, title=r"Perf. on $D_\mathrm{pub}$",
              loc="lower left",
              bbox_to_anchor=(0.31, 0.01),
              bbox_transform=ax.transAxes,
              fontsize=9, title_fontsize=9, framealpha=0.9)
    ...
    fig.savefig(out_path, dpi=150, bbox_inches="tight",
                bbox_extra_artists=[matched_legend, below_legend,
                                    *stage_label_artists])
